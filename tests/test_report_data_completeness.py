"""检测报告数据完整性检查 — 阶段6 回归测试 (standalone)
...
for f in REQUIRED_STRAIN_FIELDS:
    total += 1; passed += check(f"strain field '{f}' present in to_dict", f in d)
...
try:
    ...
    for key in ["r2_g1", "r2_g2", "slope_g1", "slope_g2", "residual_max_g1",
        ...
        total += 1; passed += check(f"charts_meta.{key} on disk", key in cm_disk)
    total += 1; passed += check("charts_meta nonlinearity_pct_g1 on disk",
                                abs(cm_disk["nonlinearity_pct_g1"] - 0.4) < 0.01)

    # 重新加载 → charts_meta 完整
    pc2 = ProjectConfig.load("报告数据测试", dir_path=tmp_dir)
    sc2 = pc2.strain["A1"]
    total += 1; passed += check("load: Ke1", abs(sc2.ke_results["Ke1"] - 1.23) < 0.001)
    total += 1; passed += check("load: Ke2", abs(sc2.ke_results["Ke2"] - 0.98) < 0.001)
    total += 1; passed += check("load: charts_meta r2_g1",
                                abs(sc2.charts_meta.get("r2_g1", 0) - 0.9995) < 0.001)
    total += 1; passed += check("load: charts_meta nonlinearity",
                                abs(sc2.charts_meta.get("nonlinearity_pct_g1", 0) - 0.4) < 0.01)
    total += 1; passed += check("load: grating_map", sc2.grating_map == {"G1": "A1-W1", "G2": "A1-W2"})
    total += 1; passed += check("load: gauge_length_mm", abs(sc2.gauge_length_mm - 80.0) < 0.01)
finally:
...
try:
    ...
    for f in REQUIRED_TEMP_FIELDS:
        total += 1; passed += check(f"temp field '{f}' present", t2 is not None and f in t2)

    # S_eff detail
    total += 1; passed += check("S_eff A1-W1 slope",
                                abs(t2["s_eff_results"]["A1-W1"]["slope"] - 27.83) < 0.01)
    total += 1; passed += check("S_eff A1-W1 r2",
                                abs(t2["s_eff_results"]["A1-W1"]["r2"] - 0.997) < 0.001)
    total += 1; passed += check("S_eff A1-W1 T_base",
                                abs(t2["s_eff_results"]["A1-W1"]["T_base"] - 14.11) < 0.01)

    # ke_table detail
    total += 1; passed += check("ke_table A1 Ke1",
                                abs(t2["ke_table"]["A1"]["Ke1"] - 0.7) < 0.01)
    total += 1; passed += check("ke_table A1 Ke2",
                                abs(t2["ke_table"]["A1"]["Ke2"] - 1.1) < 0.01)

    # decoupling detail
    total += 1; passed += check("decoupling A1 rating",
                                t2["decoupling_results"]["A1"]["rating"] == "良")
    total += 1; passed += check("decoupling A1 e_std",
                                abs(t2["decoupling_results"]["A1"]["e_std"] - 43.5) < 0.01)
finally:
...
total += 1; passed += check("full project validate passes", issues == [])

# to_dict 包含所有顶层字段
d_full = pc_full.to_dict()
for f in ["name", "created_at", "last_modified", "temperature", "strain"]:
    total += 1; passed += check(f"top-level '{f}' in to_dict", f in d_full)
