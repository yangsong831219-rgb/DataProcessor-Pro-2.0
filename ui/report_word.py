...
通过信号与主窗口通信，通过 parent() 访问 ollama_client 等共享资源。
"""
...
class WordReportWidget(QWidget):
    """Word 报告生成页面 — 双栏仪表盘布局

    Signals:
        report_generated(str): 报告生成完成时发出
        report_saved(str): 报告保存路径
    """
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        self.word_requirement_files: List[str] = []
        self.word_templates: List[str] = []
        self.word_project_files: List[str] = []
        self._setup_ui()
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        # 文件生成区域
        gen_group = self._make_group("Word文件生成", "#52c41a")
        gen_layout = QVBoxLayout(gen_group)
        gen_layout.setSpacing(10)

        self.word_gen_status = QLabel('● 就绪')
        self.word_gen_status.setStyleSheet(
            'color: #666; font-weight: bold; padding: 8px; '
            'background: #f5f5f5; border-radius: 4px;'
        )
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        clear_btn = self._make_btn('清除', '#ff4d4f', '#ff7875', lambda: self.word_ai_chat_input.clear())
        ...
        """)
        chat_layout.addWidget(self.word_ai_result, stretch=1)
    ...
    def _make_group(title: str, accent: str) -> QGroupBox:
        ...
        group.setStyleSheet(f"""
            QGroupBox {{
                ...
                padding: 10px; padding-top: 22px; background: white;
            }}
            QGroupBox::title {{
    ...
    def _on_edit_title(self) -> None:
        ...
        current = self.word_report_title.text()
        ...
        ok_btn.clicked.connect(lambda: self._save_title(title_input.text().strip(), dialog))
    ...
    def _save_title(self, title: str, dialog: QDialog) -> None:
        if title:
            self.word_report_title.setText(title)
        dialog.close()
    ...
    def _add_requirement_file(self, dialog: QDialog) -> None:
        ...
        if paths:
            settings.setValue('word_req_last_dir', os.path.dirname(paths[0]))
            for p in paths:
                if p not in self.word_requirement_files:
                    self.word_requirement_files.append(p)
    ...
    def _add_template(self, dialog: QDialog) -> None:
        ...
        path, _ = QFileDialog.getOpenFileName(dialog, '选择Word模板', last_dir,
                                              '文档文件 (*.txt *.csv *.xlsx *.docx *.doc);;所有文件 (*.*)')
        if path:
            settings.setValue('word_template_last_dir', os.path.dirname(path))
            if path not in self.word_templates:
                self.word_templates.append(path)
    ...
    def _add_project_file(self, dialog: QDialog) -> None:
        ...
        if paths:
            settings.setValue('word_project_last_dir', os.path.dirname(paths[0]))
            for p in paths:
                if p not in self.word_project_files:
                    self.word_project_files.append(p)
    ...
    def _edit_selected_file(dialog: QDialog) -> None:
        item = None
        for attr in ['_project_list', '_template_list', '_req_list']:
            w = dialog.findChild(QListWidget)
            if w and w.currentItem():
                item = w.currentItem()
                break
        if item:
            path = item.text()
    ...
    def _on_generate(self) -> None:
        ...
        try:
            ...
            project_contents = []
            for pf in self.word_project_files:
            ...
            title = self.word_report_title.text()
            if title == '未设置标题':
                title = '数据分析报告'

            prompt = (
                f"请根据以下信息生成Word格式的数据分析报告。\n\n"
                f"报告标题: {title}\n\n"
                f"需求配置:\n{chr(10).join(req_contents)}\n\n"
                f"模板文件: {self.word_templates[0]}\n\n"
                f"项目资料:\n{chr(10).join(project_contents)}\n\n"
                f"请生成完整的Word报告内容。"
            )

            # 通过 parent 链查找 ollama_client
            ollama = self._find_ollama_client()
            if ollama and ollama.is_available():
                result = ollama.generate(prompt)
                if result:
                    self._set_status('状态: 已生成Word报告', '#52c41a')
                    output = 'AI生成结果:\n' + result
                    self.word_ai_result.setPlainText(output)
                    self.report_generated.emit(output)
                else:
                    self._set_status('状态: Word报告生成失败', '#ff4d4f')
            else:
                self._set_status('状态: AI不可用，请配置Ollama服务', '#faad14')

        except Exception as e:
            self._set_status(f'状态: Word报告生成失败 - {e}', '#ff4d4f')
            import traceback
            traceback.print_exc()

    def _set_status(self, text: str, color: str) -> None:
        self.word_gen_status.setText(text)
        self.word_gen_status.setStyleSheet(
    ...
    def _on_save(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(self, '保存Word报告', last_dir,
                                              'Word Documents (*.docx)')
        if not path:
            return
        settings.setValue('word_report_save_dir', os.path.dirname(path))
        ...
        try:
            ...
            doc = Document()
            title = self.word_report_title.text()
            doc.add_heading(title if title != '未设置标题' else '数据分析报告', 0)
    ...
    def _find_ollama_client(self):
        """通过 parent 链查找主窗口的 ollama_client"""
        p = self.parent()
        while p is not None:
