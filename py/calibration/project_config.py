...
取代旧 CalibrationProfile (py/calibration/profile.py)，新增应变段。
旧 profile 作废，无需迁移。
...
class ProjectConfigManager:
    ...
    def restore(project_config: ProjectConfig, temperature_page, strain_page) -> None:
        ...
        if tp is not None and project_config.temperature is not None:
            ...
            # Step 1: annotation + groups
            tp._annotation_dict = dict(t.get("annotation", {}))
            data_cols_for_group = {}
            if tp._loaded_df is not None:
                for col_name, ann_name in t.get("annotation", {}).items():
                    try:
            ...
            # Step 5: 同步快照字典 (双副本一致)
            tp._phase_a_state = {
                "annotation": dict(t.get("annotation", {})),
