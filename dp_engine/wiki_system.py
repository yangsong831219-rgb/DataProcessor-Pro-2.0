...
class WikiFileSystem:
    ...
    def list_diagnoses(self) -> list[dict]:
        ...
        Returns:
            [{"name": str, "path": str, "size": int, "mtime": float}, ...]
            已不存在（并发删除）的文件会被跳过。
        """
        ...
        # 单次 stat — 每文件一个 try/except，一次堵死并发删除竞态
        entries: list[tuple[float, str, int]] = []  # (mtime, name, size)
        for f in sorted(diag_dir.glob("*.json")):
            try:
                st = f.stat()
            except FileNotFoundError:
                _log.debug("诊断文件已被删除，跳过: %s", f)
                continue
        ...
        results: list[dict] = [
            {"name": name, "path": str(diag_dir / f"{name}.json"),
